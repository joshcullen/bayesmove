...
Once observations from all animal IDs have been assigned to time segments, these can now be clustered to **(1)** estimate the optimal number of states, **(2)** characterize the state-specific distributions of movement variables, and **(3)** estimate the proportion of each behavior per time segment. This tutorial will show how to run the Latent Dirichlet Allocation (LDA) model, as well as how to interpret and visualize the results.
...
nmaxclust<- max(nbins) - 1  #one fewer than max number of bins used for movement variables
...
First, we will determine the optimal number of behavioral states by evaulating the proportions of time spent in each behavior across all time segments and IDs as stored within the `theta` matrix.
...
# Proportion encamped
ggplot() +
  geom_path(data = tracks4, aes(x, y, color = Encamped), size=0.5, alpha=0.7) +
  geom_point(data = tracks4 %>%
               slice(which(row_number() == 1)),
             aes(x, y), color = "green", pch = 21, size = 3, stroke = 1.25) +
  geom_point(data = tracks4 %>%
               slice(which(row_number() == n())),
             aes(x, y), color = "red", pch = 24, size = 3, stroke = 1.25) +
  scale_color_gradientn("Proportion\nEncamped",
...
# Proportion ARS
ggplot() +
  geom_path(data = tracks4, aes(x, y, color = ARS), size=0.5, alpha=0.7) +
  geom_point(data = tracks4 %>%
               slice(which(row_number() == 1)),
             aes(x, y), color = "green", pch = 21, size = 3, stroke = 1.25) +
  geom_point(data = tracks4 %>%
...
# Proportion transit
ggplot() +
  geom_path(data = tracks4, aes(x, y, color = Transit), size=0.5, alpha=0.7) +
  geom_point(data = tracks4 %>%
               slice(which(row_number() == 1)),
             aes(x, y), color = "green", pch = 21, size = 3, stroke = 1.25) +
  geom_point(data = tracks4 %>%
